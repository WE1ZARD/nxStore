...
- `changelog`: 
	- 填 `true` 时把 release 的更新内容写入 `packages/{name}/changelog.md`(仅 github_url 托管包)
- `version`: 
	- github_url 托管包**不需要填写**
	- quark 分享id, 必须填写
- `crc32`:
	- 由 `gen_manifest.py` 对 nxStore 最终托管 ZIP 自动生成,无需提交者填写
	- 上传 ZIP、单文件自动打包和 GitHub release 拉取条目都必须生成 CRC32
	- GitHub 托管包以 release tag + GitHub asset id 判断缓存；生成器私有状态保存在 `.github/github-asset-cache.json`，不进入客户端 `repo.json` / `packages/*/info.json`
	- 已发布版本仍应避免覆盖同 tag 资产；需要重发时优先提升版本号
	- 只有 `quark_url` 因仓库不持有实际归档而不生成 CRC32
- zip 条目名:
	- 入库时自动归一化为相对路径 (去前导 `/`, 反斜杠转正斜杠), 与 `packages/{name}/manifest.install` 同一坐标系
	- 客户端按 manifest 路径逐字匹配 zip 条目名; 第三方归档的 `/switch/...` 绝对路径写法不归一化会整包安装失败
```
| 分类 | 英文名 | 中文名 |
|------|--------|--------|
| emu | Emulators | 模拟器 |

...
class GenManifestRemovalTests(unittest.TestCase):
    ...
    def test_changelog_flag_writes_release_notes_into_package_dir(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            (root / "upload").mkdir()
            (root / "zips").mkdir()
            ...
            with zipfile.ZipFile(root / "zips" / "pkg.zip", "w") as archive:
                archive.writestr("switch/pkg/pkg.nro", b"current")
            (root / "packages" / "pkg" / "info.json").write_text(
                json.dumps({"version": "1.0.0"}), encoding="utf-8"
            )
            cache_path = root / ".github" / "github-asset-cache.json"
            cache_path.parent.mkdir()
            ...
            self.assertNotIn("changelog", entries[0])

    def test_changelog_requires_flag(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            (root / "upload").mkdir()
            (root / "zips").mkdir()
            ...
            self.assertFalse((root / "packages" / "pkg" / "changelog.md").exists())

    def test_empty_release_notes_keep_existing_changelog(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            (root / "upload").mkdir()
            (root / "zips").mkdir()
            ...
            self.assertEqual(
                changelog.read_text(encoding="utf-8"), "- 历史更新内容\n"
            )

    def test_manifest_install_skips_license_copyright_readme(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            (root / "zips").mkdir()
            with zipfile.ZipFile(root / "zips" / "pkg.zip", "w") as archive:
                archive.writestr("switch/pkg/pkg.nro", b"app")
                archive.writestr("LICENSE", b"license")
                archive.writestr("COPYRIGHT.txt", b"copyright")
                archive.writestr("docs/README.md", b"readme")

            GEN_MANIFEST.process_manifest(str(root), {"config"}, False, {"pkg"})

            self.assertEqual(
                (root / "packages" / "pkg" / "manifest.install").read_text(
                    encoding="utf-8"
                ),
                "U: switch/pkg/pkg.nro\n",
            )
